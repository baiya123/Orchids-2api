...
func RecordRequest(method, path, status string, durationSeconds float64) {
	RequestsTotal.WithLabelValues(method, path, status).Inc()
	RequestDuration.WithLabelValues(method, path).Observe(durationSeconds)
}

// RecordUpstream records an upstream API call.
func RecordUpstream(account, status string, durationSeconds float64) {
	UpstreamRequestsTotal.WithLabelValues(account, status).Inc()
	UpstreamDuration.WithLabelValues(account).Observe(durationSeconds)
}

// RecordTokens records token usage.
func RecordTokens(input, output int) {
	TokensProcessed.WithLabelValues("input").Add(float64(input))
	TokensProcessed.WithLabelValues("output").Add(float64(output))
}

// RecordCacheHit records a cache hit.
func RecordCacheHit(cache string) {
	CacheHits.WithLabelValues(cache, "hit").Inc()
}

// RecordCacheMiss records a cache miss.
func RecordCacheMiss(cache string) {
	CacheHits.WithLabelValues(cache, "miss").Inc()
}
